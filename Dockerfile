...
RUN addgroup -S kobrad && adduser -S kobrad -G kobrad
RUN mkdir -p /run/postgresql
RUN chown kobrad:kobrad /run/postgresql

# Tell docker that all future commands should run as the appuser user

WORKDIR /home/kobrad/releases/kobrad
COPY . .
RUN rm -rf node_modules
RUN rm package-lock.json
#RUN chown -R kobrad:kobrad .
...
ENTRYPOINT ["node","kobrad.js","--init","--nice","--mine"]

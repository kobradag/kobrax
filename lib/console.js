...
class Console {
	...
	// 		case 'local': {
	// 			return path.join(os.homedir(),'.kobrad','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
	// 		}			
	// 	}
    // }
...
*/
    ...
    // getKobrarovTasks() {
    //     return this.manager.tasks.filter(t=>t.type=="kobrarovd");
    // }

    // getKobrarovsyncTasks() {
    //     return this.manager.tasks.filter(t=>t.type=="kobrarovsyncd");
    // }

    // getKobrarovAddress() {
    //     let cfg = this.getKobrarovTasks().shift();
    //     if(!cfg)
    //         return Promise.reject(`kobrarov is not instantiated`);
    //     if(!cfg.args)
    //         return Promise.reject(`kobrarov configuration has no args object`);
    //     if(!cfg.args.listen)
    //         return Promise.reject(`kobrarov configuration arguments have no 'listen' property`);
    ...
    async native_wallet(...args) {
        ...
        let walletBin = this.manager.getDeployableBinary('kobrawallet');
        ...
        switch(cmd) {
            ...
            case 'balance': {
                //let kobrarov = await this.getKobrarovAddress();
                // let [address] = args;
                // if(!address)
                //     return Promise.reject('usage: wallet balance <address>');
                //this.log(walletBin,'balance',`/kobrarov-address:${kobrarov}`,`/address:${address}`);
                //return this.spawn(walletBin,['balance',`/kobrarov-address:${kobrarov}`,`/address:${address}`]);
            ...
            //     return this.spawn(walletBin,['send',`--kobrarov-address=${kobrarov}`,`--private-key=${private_key}`,`--to-address=${to_address}`,`--send-amount=${amount}`]);
            // } break;
            // case 'create': {
...
/*
    async migrate(cmd) {
        ...
        const kobrarovsyncdBin = path.join(this.binFolder,'kobrarovsyncd'+(process.platform == 'win32' ? '.exe' : ''));
        if(!fs.existsSync(kobrarovsyncdBin)) {
            this.log(`error: unable to locate ${kobrarovBin}`.red);
            return Promise.reject(`error: kobrarov executable not found`);
        }

        let tasks = this.getKobrarovsyncTasks();
        if(!tasks.length)
            return Promise.reject(`no kobrarovsync tasks detected`);
        ...
        // await utils.spawn(this.getBinary('kobrarovsyncd'), migrateArgs, {
        ...
        while(argsList.length) {
            ...
            await this.spawn(kobrarovsyncdBin,['--migrate', ...args], { 
...
help(term) {
    switch(term) {
        default: {
            ...
            //migrate - upgrades Kobrarov database schema\r
            this.terminal.write(`
native-wallet <command> - run native Kobra CLI wallet command, where command is:\r
\r
...
\r
rpc [command] - run RPC command against the default Kobrad instance\r
\r
    ...
    build(...args) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            this.spawn(process.argv[0], [path.join(this.appFolder,'build','build.js'), ...args], {
                ...
                let target = this.manager.getDeployableBinary('kobrad');

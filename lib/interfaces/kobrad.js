...
const {Client:KobraGRPC} = require('@kobradag/kobra-grpc');
const pkg = require("../../package");
const natUpnp = require("@runonflux/nat-upnp");

module.exports = class Kobrad extends Daemon {
	...
	start() {
		...
		return new Promise(async (resolve,reject) => {
			...
			if (!("uacomment" in args)) {
				const uacomment = `kobrax_${pkg['version']}`
				this.writeToSink(`KOBRAD: Adding uacomment=${uacomment} to arguments\n`);
			...
			if (this.task.conf['upnpEnabled'] && args['listen'] && args['listen'].indexOf(":") !== -1) {
				...
				if (externalip) {
					if (!("externalip" in args)) {
						this.writeToSink(`KOBRAD: Adding externalip=${externalip} to arguments\n`);
					...
					if (publicPort) {
						...
						this.writeToSink(`KOBRAD: Changing externalip from ${args['externalip']} to ${externalip}:${publicPort} in arguments\n`);
						args['externalip'] = `${externalip}:${publicPort}`;
						if (publicPort !== nodePort) {
							this.writeToSink(`KOBRAD: Changing listener port from ${nodePort} to ${publicPort} in arguments\n`);
							args['listen'] = `${args['listen'].split(':')[0]}:${publicPort}`;
						}
						if (!("maxinpeers" in args)) {
							this.writeToSink(`KOBRAD: Adding maxinpeers=${this.upnpMaxInPeers} to arguments\n`);
			...
			const configFile = path.join(this.folder, 'kobrad.conf');
			...
			await this.VSARGS([this.getDeployableBinary('kobrad'),'--version']);

			this.run({
				args : async () => {
					let _args_ = [
 						this.getDeployableBinary('kobrad')
					];
			...
			resolve();
			// mutes all kobrad processes by default
			this.mute = true;
		})
	}
		...
		return new Promise(async (resolve, reject) => {
	        ...
	        try {
				...
				this.data = this.sourcesToData(sources);
	             ...
	             console.log('KOBRAD monitor:'.bold,ex);
	            return reject(ex);
	        }

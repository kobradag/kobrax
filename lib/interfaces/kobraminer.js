...
module.exports = class Simulator extends Daemon {
	...
	async start() {
		...
		let defaults = {
			...
			// dbuser: 	'kobra', // Database user
			// dbpass: 	'kobra', // Database password
			// dbname: 	'kobrarov', // Database name
...
//		let {config:daemons} = await this.get("get-modules-config");
		...
		if(!args.miningaddr) {
			args.miningaddr = 'kobratest:qpjmmd8fc20c6mf5zxr6qax0369g0aufyg6aqcel9u';
			//args.miningaddr = 'kobra:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
			//args.miningaddr = 'kobra:qr35ennsep3hxfe7lnz5ee7j5jgmkjswsslwxj42ta';
		}		
		...
		//console.log("++++++++++++++++ KOBRAMINER VSARGS")

		await this.VSARGS([this.getDeployableBinary('kobraminer'), '--version']);

		args = Object.entries(args);

		const flags = utils.args();
		//console.log("++++++++++++++++ KOBRAMINER FLAGS",flags)

		this.options = {
			args : () => {
				// this.log(`--addresslist=${configFile}`,`--cert=${rpcCertFile}`);
				let _args_ = [
					this.getBinary('kobraminer'),
			...
			stdout : (data) => { 
				...
				// // console.log("kobraminer match:", match);
		...
		//console.log("KOBRAMINER STATUS:",this.manager.enableMining,flags.mine);
		if(this.manager.enableMining || flags.mine) {
			dpc(0, () => {
				//console.log("----------------- KOBRAMINER STARTING")
				this.run()
			})
		} else {
			//console.log("----------------- KOBRAMINER NOT RUNNING")
			this.state = 'disabled';
			dpc(0, () => {
				this.writeToSink('\r\n\r\nKobraminer is disabled. Please use Enable Mining option to start it.\r\n\r\n');
			})
		}

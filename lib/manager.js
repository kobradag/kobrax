...
class KobraProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['kobrad','mqtt','mysql','pgsql', 'txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['kobrad','mqtt','mysql','pgsql','simulator','txgen'];
		//const typeMap = { };
	...
	getDeployableBinary(...args) {
		...
		switch(buildType) {
			...
			case 'local': {
				return path.join(os.homedir(),'.kobrax','bin',utils.platform, ...args)+this.PLATFORM_BINARY_EXTENSION;
			}			
		}
	}
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, kobra } = pkg;
			return {
				folder, name, description, main, engines, ...kobra
			}
		});
	}

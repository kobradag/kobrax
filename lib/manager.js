...
class KOBRAProcessManager extends EventEmitter{
	...
	async start(daemons={}) {
		...
		const startupTypeOrder = ['kobrad','mqtt','mysql','pgsql','kobrarovsyncd','kobrarovd', 'gpuminer', 'kobraminer','txgen','app'];
		const typeMap = { };
	...
	async stop() {
		//const shutdownTypeOrder = ['kobrad','mqtt','mysql','pgsql','kobrarovsyncd','kobrarovd','simulator','txgen'];
		//const typeMap = { };
	...
	enumerateApps() {
		...
		return packages.map(({ folder, pkg }) => {
			let { name, description, main, engines, kobrad } = pkg;
			return {
				folder, name, description, main, engines, ...kobrad
			}
		});
	}
...
module.exports = KOBRAProcessManager;

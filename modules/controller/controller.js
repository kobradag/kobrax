...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'kobrad'});
		this.rpc = rpc;
	...
	async initConsole() {
		let terminal = this.qS('#kobrad-console');
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "kobrad.$HOSTNAME";
...
${changelogContent}`;
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.kobrad) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/kobrad-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://kobrad.app/version.json';
		...
		if(!data) {
			console.log("missing version data in https://kobrad.app/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://kobrad.app');

				let confirm = await FlowDialog.show({
					title:i18n.t("KDX Update"),
					body:html`Please download the latest version from  <flow-link href="https://kobrad.app" target="_blank">https://kobrad.app</flow-link>
					<br/>&nbsp;<br/>

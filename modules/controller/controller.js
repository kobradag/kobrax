...
const {RPC} = require('@kobradag/kobra-grpc');
...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@kobradag/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@kobradag/ux"] = "N/A";
window.PWA_MODULES["@kobradag/grpc-web"] = "N/A";
window.PWA_MODULES["@kobradag/wallet"] = "N/A";
window.PWA_MODULES["@kobradag/grpc"] = "N/A";
window.PWA_MODULES["@kobradag/core-lib"] = "N/A";

class KobraDesktopApp extends FlowApp{
	render(){
		let walletMeta = {"generator":"Kobrax"}
		let list = [
			['Kobra','MIT','Copyright (c) 2023 Kobra Developers'],
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		let copyright = [
			['Kobrax','Copyright (c) 2024 Kobra Developers', 'All Rights Reserved'],
			['KDX','Copyright (c) 2020 Kaspa Developers', 'All Rights Reserved']
		];
		let donationAddresses = [
			["Kobra Devfund donations:", "kobra:qpscc6kzu2ny8ga7tr72csftgxjnmzc0658355tgs9k45qpn0vm0qzlfenraj"],
		]
		return html`
		<flow-caption-bar
			logo="/resources/images/kobra-logo-light-bg.png">Kobrax</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='kobra-resources'>
				...
				<flow-expandable>
					<div slot="title" is="i18n-div" caption>KOBRADAG RESOURCES</div>
					<ul style="font-size: 12px;">
						<li><flow-shell-link
							href="https://github.com/kobradag/"><flow-i18n>GitHub</flow-i18n></flow-shell-link></li>
						<li><flow-shell-link
							href="https://discord.gg/BeNxna8YZ2"><flow-i18n>Discord Chat</flow-i18n></flow-shell-link></li>
						<li><flow-link
		...
		<tab-content for="settings">
			<flow-form-control icon="fal:database">
				...
				<p slot="info" is="i18n-p">
					Data Folder location is used for storage by all Kobrax
					modules. In default configuration this includes Kobrad blockchain
			...
			<flow-form-control icon="fal:tools">
				...
				<p slot="info" is="i18n-p">
					Advanced mode allows you to manually configure, interact with and monitor Kobrax services.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					When enabled, Kobrax runs itself and it's services hidden in the background
			...
			<flow-form-control icon="fal:cog" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">When Autocompound enabled, Kobrax compound UTXOs when there will be compoundable count of unspent transaction outputs (UTXOs).'Use latest Change address' will compund UTXOs using current/latest change address instead of first change address. </p>
			</flow-form-control>
			<!-- flow-form-control icon="fal:drafting-compass" class="advanced-tool">
				...
				<p slot="info" is="i18n-p">
					You can stream Kobrax metrics to your own StatsD-compatible server.
				</p>
			</flow-form-control-->
			...
			<flow-form-control id="settings-script" icon="fal:tasks">
				...
				<p slot="info" is="i18n-p">
					Configuration editor allows you to customize Kobrax environment.
					Kobrax configuration is represented in JSON
			...
			<flow-form-control id="settings-templates" icon="fal:file-alt"
				...
				<p slot="info" is="i18n-p">
					Configuration templates allow you to load pre-made Kobrax configurations.
				</p>
			</flow-form-control>
			<flow-form-control icon="fal:database">
...
//		this.checkForUpdates().then();
	...
	initRPC(){
		let rpc = new FlowRPC({bcastChannel:'kobrax'});
		this.rpc = rpc;
	...
	async get_default_local_kobrad_settings() {
		...
		if(!kobrad)
			return null;//{network:"kobra", port:42110};//{network:"kobratest", port:42110};
	...
	async initSettings(){
		...
		if(statsdPrefixInput)
			this.statsdPrefix = statsdPrefixInput.value = config.statsdPrefix || "kobrax.$HOSTNAME";
	...
	initReleaseNotes(){
		...
		dialog.content = 
`#	Welcome to Kobrax ${pkg.version}!

Useful resources:
- Kobra Discord: https://discord.gg/ZPZRvgMJDT
- Kobra GitHub: https://github.com/kobradag/
- Kobrax GitHub: https://github.com/kobradag/kobrax
...
${changelogContent}`;
	...
	showApps(daemons) {
		...
		let entries = Object.values(appsMap).map((app) => {
			...
			if(!location && app.engines?.kobrax) {
			...
			return `${this.debugAppLinks?JSON.stringify(app)+"::"+location:""}
				<flow-window-link
					...
					width="${width}"
					height="${height}"
					icon="resources/images/kobrax-icon.png"
					resizable
					frame
	...
	async checkForUpdates(force = false) {
		...
		const url = 'https://k0bradag.com/kobrax/version.json';
		...
		if(!data) {
			console.log("missing version data in https://k0bradag.com/kobrax/version.json");
			return false;
		}
		...
		if(is_gt) {
			...
			if(btn == 'ok') {
				require('nw.gui').Shell.openExternal('https://k0bradag.com');

				let confirm = await FlowDialog.show({
					title:i18n.t("Kobrax Update"),
					body:html`Please download the latest version from  <flow-link href="https://k0bradag.com" target="_blank">https://k0bradag.com</flow-link>
					<br/>&nbsp;<br/>
					Kobrax will now shutdown`,
					btns:[{
						text:i18n.t('Cancel'),
						value:'cancel'
					},{
...
KobraDesktopApp.define("kobrax-app")
